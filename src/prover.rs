...
pub struct Prover2<S = Setup>
where
    S: State,
{
    poseidon: Box<dyn Hash>,
    prover: Box<dyn ProveTrait>,
    state: S,
}
...
impl Prover2<Setup> {
    ...
    // chunk.
    pub fn setup(self) -> Result<Prover2<PlaintextCommitment>, ProverError> {
...
impl Prover2<PlaintextCommitment> {
    pub fn plaintext_commitment(
        self,
    ) -> Result<(Vec<BigUint>, Prover2<LabelsumCommitment>), ProverError> {
...
impl Prover2<LabelsumCommitment> {
    ...
    pub fn labelsum_commitment(
        ...
        labels: &Vec<u128>,
    ) -> Result<(Vec<BigUint>, Prover2<ProofCreation>), ProverError> {
        ...
        Ok((
            ...
            Prover2 {
                state: ProofCreation {
                    ...
                    chunk_size: self.state.chunk_size,
                    labelsum_hashes,
                    hashes_of_chunks: self.state.hashes_of_chunks,
                },
                poseidon: self.poseidon,
                prover: self.prover,
            },
...
impl Prover2<ProofCreation> {
    ...
    fn create_proof_inputs(&self, zero_sum: Vec<BigUint>, mut deltas: Vec<BigUint>) -> Vec<String> {
        ...
        for count in 0..self.state.chunks.len() {
            ...
            // prepare input.json
            let input = object! {
                plaintext_hash: self.state.hashes_of_chunks[count].to_string(),
