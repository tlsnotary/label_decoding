...
pub struct ProofInput {
    ...
    pub label_sum_hash: BigUint,
    pub sum_of_zero_labels: BigUint,
    pub plaintext: Vec<BigUint>,
    pub salt: BigUint,
    pub deltas: Vec<BigUint>,
}
...
pub struct Setup {
    ...
    proving_key: Box<dyn ProvingKeyTrait>,
...
/// the trait which must be implemented by the backend's proving key
/// type.
pub trait ProvingKeyTrait {}
...
impl LabelsumProver<ProofCreation> {
    ...
    fn create_proof_inputs(
        ...
        // the corresponding deltas of the last chunk.
        let delta_pad_count = self.state.chunk_size * self.state.chunks.len() - deltas.len();
        deltas.extend(vec![BigUint::from(0u8); delta_pad_count]);
